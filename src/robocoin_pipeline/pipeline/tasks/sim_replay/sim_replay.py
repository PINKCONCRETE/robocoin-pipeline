...
class LerobotSimReplayer:
    def __init__(
        ...
        # 只在非 headless 模式下初始化 Renderer 和 Camera
    ...
    def get_img(self):
        if self.headless or self.renderer is None:
            raise RuntimeError("Cannot get image in headless mode - renderer not initialized")
...
def main():
    ...
    try:
        while replayer.step():
            # 只在非 headless 模式下获取和保存图像
            if not args.headless:
                # 获取当前帧图像
                img_rgb = replayer.get_img()

                # 保存图片 (OpenCV 使用 BGR 格式)
                if frame_idx % 1 == 0:  # 每5帧存一张，避免太多
